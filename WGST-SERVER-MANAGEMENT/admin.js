...
function renderDashboard() {
  ...
  tbody.innerHTML = recent.map(s=>`
    <tr>
      ...
      <td>${s.items.map(x=>x.name).join(', ').substring(0,22)}…</td>
  ...
  document.getElementById('lowStockList').innerHTML = lowItems.length ? lowItems.map(i=>`
    <div class="alert-item ${i.stock===0?'critical':''}">
      <div>
        ...
        <div class="alert-item-stock">${i.stock===0?'OUT OF STOCK':`${i.stock} / ${i.threshold} min`} · ${i.sku}</div>
      </div>
...
function renderReports() {
  ...
  if (type === 'all') {
    // Show every item that has at least a refilling or expiry date
    filteredItems = filteredItems.filter(item => item.refillingDate || item.nextDurationDate);

  } else if (type === 'due_soon') {
    // Items with nextDurationDate within the next 30 days (including today)
    ...
    // Items whose nextDurationDate has already passed
    filteredItems = filteredItems.filter(item => {
      const dl = getDaysLeft(item);
      return dl !== null && dl < 0;
    });
    ...
    // Filter by the EXPIRY date month
    ...
    // Filter by a specific EXPIRY date
    ...
    // Filter by EXPIRY date within a range
